...
class UserActor(userId: String) extends Actor {
  ...
  def receive = {
    case userMessage: UserMessage =>
      log.info(s"received test ${userMessage.value}")
    ...
    case SendNow => {
      currentMessage.foreach { message =>
        context.parent ! SendMessage(currentMessage)
      }
...
object UserActor {
  ...
  case class UserMessage(value: String)

  def props(magicNumber: String): Props = Props(new UserActor(magicNumber))
